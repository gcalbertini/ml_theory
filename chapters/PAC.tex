...
\exercise{2.7}{
	\begin{itemize}
		...
		\item We use Hoeffding's inequality. Recall that $d(h^*) =
			      \eta$ is a
		      random variable and also $\mathbb{E}[\hat{d}(h^*)] = \sum_i x_i
			      \eta/m \leq \eta = d(h^*)$, as the expectation of empirical risk is the true risk for i.i.d random variables (getting a sum of up to $m$ 1's for our binary classification purposes), as defined in the problem.
		      \begin{flalign*}
