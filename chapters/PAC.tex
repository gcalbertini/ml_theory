...
\exercise{2.1}{
	...
	most $\epsilon$. Assume a distribution $\mathcal{D}$ on $\mathcal{X}
		\times \{-1, +1\}$ where both oracles generate samples from
	$\mathcal{D}_{+}
		\propto \mathcal{D}(x,1)$ and $\mathcal{D}_{-} \propto
		\mathcal{D}(x,-1)$.
	...
	\begin{flalign*}
		R(h)_{\mathcal{D}} & = \underset{x \sim
			...
			c(x)]
		\\
		& =\mathcal{D}^+(R(h)_{\mathcal{D}_{+}})+\mathcal{D}^-(R(h)_{\mathcal{D}_{-}})
		\\
		& =\mathcal{D}^+(R(h)_{\mathcal{D}_{+}})+(1-\mathcal{D}^+)(R(h)_{\mathcal{D}_{-}})
		\\
	\end{flalign*}
	Now for a weighted sampling method from the negative and positive
	instance distributions we can say $\epsilon_{\mathcal{D}} =
		\mathcal{D}^+(\alpha
		\epsilon)_{\mathcal{D}^+}+\mathcal{D}^-(\beta
		\epsilon)_{\mathcal{D}^-} =  \epsilon
		\underbrace{(\mathcal{D}^+(\alpha -
	...
	Thus, select an
	appropriate $\delta$ and, for simplicity, set $\mathcal{D}^+ =
		\frac{1}{2}$, to get
	\begin{flalign*}
	...
	in $p(1/\epsilon, 1/δ, size(c))$, such that if we draw at least this
	number of negative and positive instances with confidence $1 - \delta$,
	the
	...
	to take $m \geq \max\{m^+, m^-\}$ and, with conditions above,
	\begin{flalign*}
		\mathbb{P}[R(h)_{\mathcal{D}}]
	\end{flalign*}
